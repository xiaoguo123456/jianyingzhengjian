...
| [DATA_MODEL.md](./DATA_MODEL.md) | PostgreSQL schema, invariants and migration rules |
...
│   └── deploy/           # release Dockerfile, compose files, gateway snippets, deploy/rollback scripts
└── .github/workflows/    # backend check, test auto-deploy, production manual deploy
```

Not built yet: the admin console (React + Ant Design, optional in V1.0); catalogue and config are managed through the admin API meanwhile.
