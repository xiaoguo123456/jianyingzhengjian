<template>
  <view class="cc" :class="{ 'cc--compact': compact }">
    <view class="cc__main" :hover-class="tappable ? 'cc--hover' : 'none'" @tap="$emit('open')">
      <view class="cc__info">
        <view v-if="!compact" class="cc__label">生成次数</view>
        <view class="cc__amount">
          <text v-if="compact" class="cc__available">可用</text>
          <text class="cc__num num">{{ credits?.total ?? '—' }}</text>
          <text class="cc__unit">次</text>
        </view>
        <view v-if="!compact" class="cc__cap">{{ caption }}</view>
      </view>
...
const props = withDefaults(defineProps<{ credits: Credits | null; tappable?: boolean; inviteText?: string; compact?: boolean }>(), { tappable: false, inviteText: '' })
...
.cc { background: $color-surface; border-radius: $radius-lg;  overflow: hidden; }
...
.cc__btn {
  ...
  border-radius: 18rpx; background: $color-primary; color: #fff; font-size: 26rpx; font-weight: 600;
...
.cc--compact .cc__main { padding: 24rpx; }
.cc--compact .cc__amount { margin: 0; gap: 10rpx; }
.cc__available { font-size: 26rpx; color: $color-text-2; }
.cc--compact .cc__num { font-size: 40rpx; font-weight: 600; letter-spacing: 0; }
.cc--compact .cc__unit { font-size: 26rpx; font-weight: 400; }
.cc--compact .cc__btn { height: 76rpx; padding: 0 22rpx; font-size: 24rpx; font-weight: 500; }
.cc--compact .cc__invite { height: 82rpx; padding: 0 24rpx; }
.cc--compact .cc__invite-icon { background: transparent; width: 32rpx; }
.cc--compact .cc__invite-text { font-size: 24rpx; }
</style>
