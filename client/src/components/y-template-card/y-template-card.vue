<template>
  <view class="tc" hover-class="tc--hover" @tap="$emit('tap', template)">
    <view class="tc__cover-wrap" :class="{ 'tc__cover-wrap--square': square }">
      ...
      <view v-if="template.tags.length" class="tc__tag"><y-tag :text="template.tags[0]" :variant="template.tags[0] === '热门' ? 'hot' : 'primary'" /></view>
...
defineProps<{ template: TemplateCard; square?: boolean }>()
defineEmits<{ (e: 'tap', t: TemplateCard): void }>()
</script>

<style lang="scss" scoped>
/* Image + name only (PRD 16). No card chrome: the photo is the card. */
...
.tc__name { padding: 14rpx 4rpx 4rpx; font-size: $font-body; font-weight: 600; color: $color-text; }
</style>
